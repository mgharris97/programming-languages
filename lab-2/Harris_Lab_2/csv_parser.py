...
def csv_parse(file_path):
    ...
    with open(file_path, newline="", mode="r") as f:
        ...
        for line_num, i in enumerate(csv_file, start=1):
            ...
            if i[0].startswith("#"):
                errors_list.append(
                    f"Line {line_num} of {os.path.basename(file_path)}: {i[0]} → comment line, ignored for parsing"
                )
                continue
            if len(i) != 6:
                errors_list.append(
                    f"Line {line_num} of {os.path.basename(file_path)}: {i} → invalid format, have six cells"
                )
                continue
            ...
            if not (2 <= len(i[0]) <= 8 and i[0].isalnum):
                errors_list.append(
                    f"Line {line_num} of {os.path.basename(file_path)}: {i}:'{flight_id}' → flight ID not 2-8 alphanumeric character"
                )
            ...
            if not (
                ...
                and len(origin) == 3
                and origin.isalpha()
                and len(set(origin)) > 1
            ):
                errors_list.append(
                    f"Line {line_num} of {os.path.basename(file_path)}: {i}: '{origin}' → Invalid origin (not 3 uppercase, non-repeating letters)"
                )
                
            if not (
                destination.isupper()
                and len(destination) == 3
                and destination.isalpha()
                and len(set(destination)) > 1
            ):
                errors_list.append(
                    f"Line {line_num} of {os.path.basename(file_path)}: {i}: '{destination}' → Invalid destiantion (not 3 uppercase letters)"
                )
                #continue
            try:
            ...
            if not (is_departure_date):
                errors_list.append(
                    f"Line {line_num} of {os.path.basename(file_path)}: {i}: '{departure}' → Invalid departure datetime"
                )
                #continue
            try:
            ...
            try:
                if not (is_arrival_date and (arrival_time > departure_time)):
                    errors_list.append(
                        f"Line {line_num} of {os.path.basename(file_path)}:{i}: '{arrival}' → Arrival time occurs before departure → {departure}"
                    )
                    #continue
            except ValueError:
                errors_list.append(
                    f"Line {line_num} of {os.path.basename(file_path)}: {i}: '{arrival}' → Invalid arrival time"
                )
                #continue
            ...
            try:
                ...
                if flight_price < 0:
                    errors_list.append(
                        f"Line {line_num} of {os.path.basename(file_path)}: {i}: '{price}' → Price cannot be negative. Aint no one here paying you to fly w us"
                    )
                    #continue
            except ValueError:
                errors_list.append(
                    f"Line {line_num} of {os.path.basename(file_path)}: {i}: '{price}' → Price must be a valid float"
                )
                #continue
